...
describe("LB102 real-case regression corpus",()=>{
  ...
  it("conserva rasgos exclusivos de cada subfamilia",()=>{
    ...
    expect(panda.invariants.divisionIntoLots).toBe(false);
    expect(panda.invariants.durationMonths).toBe(36);
    expect(panda.invariants.cpvMain).toBe("48760000-3");
    expect(panda.invariants.partnerGoldRequired).toBe(true);
  ...
  it("clasifica como defecto una desviación del dato fuente",()=>{
    const panda=findRealCase("REG-SUPPLY-002")!;
    const diff=compareRealCase(panda,Object.entries(panda.invariants).map(([key,value])=>({key,value:key==="durationMonths"?12:value})));
    expect(diff.some(x=>x.key==="durationMonths"&&x.category==="ERROR_CONTRATA_IA")).toBe(true);
