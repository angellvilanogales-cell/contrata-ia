...
describe("LB102 Service V2 source-structural gate",()=>{
 ...
 it("no acredita una plantilla estructural demasiado corta frente a fuentes de 13/28 y 111/50 páginas",()=>{
  expect(LB102_SERVICE_PHYSICAL_SOURCE_PROFILE["service-huelva"].documents.MEMORIA?.sourcePages).toBe(13);expect(LB102_SERVICE_PHYSICAL_SOURCE_PROFILE["service-huelva"].documents.PPT?.sourcePages).toBe(28);expect(LB102_SERVICE_PHYSICAL_SOURCE_PROFILE["service-5g"].documents.PCAP?.sourcePages).toBe(111);expect(LB102_SERVICE_PHYSICAL_SOURCE_PROFILE["service-5g"].documents.PPT?.sourcePages).toBe(50);
  for(const id of ["service-huelva","service-5g"] as const){const policy=LB102_SOURCE_FIDELITY_POLICY[id];expect(policy.level).toBe("DERIVED_STYLE_PENDING_COMPARISON");expect(policy.accredited).toBe(false);expect(servicePhysicalSourceBlockers(id).length).toBeGreaterThan(0);expect(LB102_PILOT_PACKAGE_CATALOG.find(x=>x.id===id)?.profile).toBe("SERVICE_SOURCE_STRUCTURAL_PILOT_LB102_V2");}
 });
