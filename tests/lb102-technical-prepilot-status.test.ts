...
describe("LB102 technical pre-pilot status",()=>{
 it("deriva 2 Supply + 2 Service del corpus y queda técnicamente listo cuando LB101 preflight está acreditado",()=>{
  ...
  const status=evaluateLB102TechnicalPrePilot({lb101SecurityReady:true,...negatives});expect(status.technicalPrePilotReady).toBe(true);expect(status.appViableForPilot).toBe(false);expect(status.productionReady).toBe(false);
 ...
 it("no autoafirma seguridad del despliegue",()=>{
  const status=evaluateLB102TechnicalPrePilot({lb101SecurityReady:false,...negatives});expect(status.technicalPrePilotReady).toBe(false);expect(status.blockers).toContain("LB101 debe acreditar operación multiusuario segura para piloto.");
 });
});
