...
const keys=["NODE_ENV","CONTRATA_IA_USERS_JSON","CONTRATA_IA_PUBLIC_BASE_URL","RENDER_EXTERNAL_URL","CONTRATA_IA_PERSISTENCE_URL","CONTRATA_IA_PERSISTENCE_TOKEN","CONTRATA_IA_DATA_DIR"] as const;
...
describe("LB102 live preflight",()=>{
 ...
 it("reconoce la terminación TLS nativa de Render aunque no se sincronice la URL pública propia",()=>{const root=configure();delete process.env.CONTRATA_IA_PUBLIC_BASE_URL;process.env.RENDER_EXTERNAL_URL="https://contrata-ia.onrender.com";try{const status=runLB101LivePreflight();expect(status.pilotSecurityReady).toBe(true);expect(status.blockers).not.toContain("El piloto debe operar detrás de HTTPS/TLS.");}finally{fs.rmSync(root,{recursive:true,force:true});}});
