...
describe("LB91.39-40 - cobertura documental basada en fuentes", () => {
  ...
  it("reconoce el PPT general de servicios sin perder el contraste multifuente", () => {
  ...
  it("declara lista la terna física general de servicios", () => {
    ...
    expect(pcap?.status).toBe("GENERAL_EDITABLE");
    expect(pcap?.physicalUniversalGenerationReady).toBe(true);
  });
});
