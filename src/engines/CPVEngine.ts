...
export class CPVEngine extends BaseEngine {
    ...
    public ejecutar(contexto: ExpedienteContext): DecisionJuridica<CPVEntry[]> {
        ...
        const candidatos = this.matcher.buscar(contexto.objeto, catalogo, 10);
        const decision = new DecisionJuridica<CPVEntry[]>();
        decision.resultado = candidatos.map(c => c.cpv);

        const primero = candidatos[0];
        if (primero && decision.resultado.length > 0) {
            contexto.cpvPrincipal = decision.resultado[0];
            contexto.cpvSecundarios = decision.resultado.slice(1);
            decision.confianza = primero.puntuacion;
        } else {
            contexto.cpvSecundarios = [];
            decision.confianza = 0;
        }

        decision.explicacion = "Selección automática de códigos CPV basada en coincidencia semántica.";
        decision.reglasAplicadas.push("CPV-001");
        return decision;
    }
}
