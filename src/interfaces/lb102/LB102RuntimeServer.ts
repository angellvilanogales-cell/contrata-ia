...
import {createLB102PilotAcceptanceStoreFromEnv,type LB102PilotAcceptanceStoreLike} from "../../application/operations/lb102/LB102PilotAcceptanceStore";
...
import {assertReviewedPackageSha} from "../../application/operations/lb102/LB102PilotReviewIntegrity";
...
/** Runtime LB102: conserva LB99 y añade preflight, autodiagnóstico, ingreso binario protegido y aceptación humana auditable y durable. */
export function createLB102RuntimeServer():http.Server{
 const base=createLB99RuntimeServer();const security=new SecurityPolicy();const dataRoot=path.resolve(process.env.CONTRATA_IA_DATA_DIR??"var/contrata-ia");const acceptance=createLB102PilotAcceptanceStoreFromEnv(path.join(dataRoot,"lb102","acceptance.json"));
 let uatSimulationCache:Awaited<ReturnType<typeof runLB102PreHumanMachineSimulation>>|null=null;
 const downloadedPackageSha=new Map<string,string>();
 const invalidateUatCache=()=>{uatSimulationCache=null;downloadedPackageSha.clear();};
 const currentUatSimulation=async()=>{if(uatSimulationCache?.passed)return uatSimulationCache;const simulation=await runLB102PreHumanMachineSimulation();if(simulation.passed)uatSimulationCache=simulation;return simulation;};
 const requireCurrentUatSimulation=async()=>{const simulation=await currentUatSimulation();if(!simulation.passed)throw new Error(`SIMULACIÓN TÉCNICA PREVIA NO SUPERADA: ${simulation.blockers.join(" | ")}`);return simulation;};
 const downloadKey=(actorId:string,packageId:string)=>`${actorId}:${packageId}`;
 return http.createServer(async(request,response)=>{security.applySecurityHeaders(response);try{const url=new URL(request.url??"/","http://localhost");const parts=url.pathname.split("/").filter(Boolean);
  ...
  if(parts[0]==="api"&&parts[1]==="lb102"&&parts[2]==="pilot-packages"){
   ...
   if(request.method==="GET"&&parts[4]==="download"&&parts.length===5){const pkg=await generateLB102PilotPackage(descriptor.id);if(!pkg.ready||!pkg.bytes||!pkg.fileName||!pkg.sha256){sendJson(response,409,{ready:false,blockers:pkg.blockers,productionReady:false});return;}downloadedPackageSha.set(downloadKey(actor.id,descriptor.id),pkg.sha256);sendZip(response,pkg.bytes,pkg.fileName,pkg.sha256);return;}
   if(request.method==="POST"&&parts[4]==="review"&&parts.length===5){const expectedSha=downloadedPackageSha.get(downloadKey(actor.id,descriptor.id));if(!expectedSha){sendJson(response,409,{error:"No consta un ZIP generado y descargado por este usuario en el runtime actual. Genera y revisa de nuevo el paquete antes de registrar la revisión.",productionReady:false});return;}await requireCurrentUatSimulation();const pkg=await generateLB102PilotPackage(descriptor.id);if(!pkg.ready||!pkg.sha256){sendJson(response,409,{ready:false,blockers:pkg.blockers,productionReady:false});return;}try{assertReviewedPackageSha(expectedSha,pkg.sha256);}catch(error){downloadedPackageSha.delete(downloadKey(actor.id,descriptor.id));sendJson(response,409,{error:error instanceof Error?error.message:String(error),expectedPackageSha256:expectedSha,actualPackageSha256:pkg.sha256,productionReady:false});return;}const body=await readJson(request) as Record<string,unknown>;const review=await acceptance.recordReview({caseId:descriptor.caseId,family:descriptor.family,packageSha256:expectedSha,accepted:body.accepted===true,criticalDefectsOpen:Number(body.criticalDefectsOpen??0),notes:typeof body.notes==="string"?body.notes:undefined},actor.id);sendJson(response,201,{review,package:{id:descriptor.id,caseId:descriptor.caseId,family:descriptor.family,sha256:expectedSha},productionReady:false});return;}
  }
  if(url.pathname.startsWith("/api/lb102/acceptance")){const actor=security.authenticate(request);if(request.method==="GET"&&url.pathname==="/api/lb102/acceptance/status"){security.require(actor,"VIEWER");const lb101=runLB101LivePreflight();const simulation=await currentUatSimulation();const evidence=await acceptance.evidence();sendJson(response,200,{...(await acceptanceStatus(acceptance,lb101.pilotSecurityReady)),preHumanMachineSimulation:simulation,evidence,productionReady:false});return;}
   if(actor.namedIdentity!==true)throw new Error("LB102 exige identidad nominativa para registrar aceptación humana.");
   if(request.method==="POST"&&url.pathname==="/api/lb102/acceptance/sessions"){security.require(actor,"REVIEWER");await requireCurrentUatSimulation();sendJson(response,201,await acceptance.recordSession(actor.id));return;}
   ...
   if(request.method==="POST"&&url.pathname==="/api/lb102/acceptance/decision"){security.require(actor,"ADMIN");await requireCurrentUatSimulation();const body=await readJson(request) as Record<string,unknown>;sendJson(response,201,await acceptance.recordDecision({accepted:body.accepted===true,rationale:String(body.rationale??"")},actor.id));return;}
  }
