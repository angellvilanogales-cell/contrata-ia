...
export function createLB102RuntimeServer():http.Server{
 ...
 return http.createServer(async(request,response)=>{security.applySecurityHeaders(response);try{const url=new URL(request.url??"/","http://localhost");const parts=url.pathname.split("/").filter(Boolean);
  ...
  if(parts[0]==="api"&&parts[1]==="lb102"&&parts[2]==="pilot-packages"){
   ...
   if(request.method==="POST"&&parts[4]==="review"&&parts.length===5){await requireLB102PreHumanMachineSimulation();const pkg=await generateLB102PilotPackage(descriptor.id);if(!pkg.ready||!pkg.sha256){sendJson(response,409,{ready:false,blockers:pkg.blockers,productionReady:false});return;}const body=await readJson(request) as Record<string,unknown>;const review=await acceptance.recordReview({caseId:descriptor.caseId,family:descriptor.family,packageSha256:pkg.sha256,accepted:body.accepted===true,criticalDefectsOpen:Number(body.criticalDefectsOpen??0),notes:typeof body.notes==="string"?body.notes:undefined},actor.id);sendJson(response,201,{review,package:{id:descriptor.id,caseId:descriptor.caseId,family:descriptor.family,sha256:pkg.sha256},productionReady:false});return;}
  }
  if(url.pathname.startsWith("/api/lb102/acceptance")){const actor=security.authenticate(request);if(request.method==="GET"&&url.pathname==="/api/lb102/acceptance/status"){security.require(actor,"VIEWER");const lb101=runLB101LivePreflight();const simulation=await runLB102PreHumanMachineSimulation();const evidence=await acceptance.evidence();sendJson(response,200,{...(await acceptanceStatus(acceptance,lb101.pilotSecurityReady)),preHumanMachineSimulation:simulation,evidence,productionReady:false});return;}
   if(actor.namedIdentity!==true)throw new Error("LB102 exige identidad nominativa para registrar aceptación humana.");
   if(request.method==="POST"&&url.pathname==="/api/lb102/acceptance/sessions"){security.require(actor,"REVIEWER");await requireLB102PreHumanMachineSimulation();sendJson(response,201,await acceptance.recordSession(actor.id));return;}
   ...
   if(request.method==="POST"&&url.pathname==="/api/lb102/acceptance/decision"){security.require(actor,"ADMIN");await requireLB102PreHumanMachineSimulation();const body=await readJson(request) as Record<string,unknown>;sendJson(response,201,await acceptance.recordDecision({accepted:body.accepted===true,rationale:String(body.rationale??"")},actor.id));return;}
  }
