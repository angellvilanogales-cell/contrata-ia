...
if(["initialBudgetExVat","initialDurationMonths","initialOneOffCostExVat","recurringAnnualCostExVat"].includes(id)){var n=Number(rawValue.replaceAll(".","").replace(",","."));if(!Number.isFinite(n))throw new Error("Indique una cifra válida.");return n;}
return rawValue;
}
...
async function analyze(){try{var d=await callAnalyze();status.textContent="Sesión autenticada.";var html=renderDecision(d);if(d.nextQuestion){html+='<div class="card"><h2>Siguiente pregunta</h2><h3>'+esc(d.nextQuestion.label)+'</h3><p>'+esc(d.nextQuestion.help)+'</p>'+inputFor(d.nextQuestion)+'<button id="saveAnswer">Guardar y continuar</button></div>';}else{html+='<div class="card info"><h2>Espina dorsal completada</h2><p>Ya pueden abrirse las ramas específicas que correspondan.</p></div>';}work.innerHTML=html;if(d.nextQuestion){document.getElementById("saveAnswer").addEventListener("click",async function(){var el=document.getElementById("ans");if(!el||!el.value.trim()){alert("Indique una respuesta.");return;}try{answers[d.nextQuestion.id]=parseValue(d.nextQuestion.id,el.value.trim());sessionStorage.setItem("contrataIaAdaptiveAnswers",JSON.stringify(answers));await analyze();}catch(e){alert(e.message);}});}}catch(e){status.textContent="Sesión no autenticada o error de acceso.";work.innerHTML='<div class="card warning">'+esc(e.message)+'</div>';}}
