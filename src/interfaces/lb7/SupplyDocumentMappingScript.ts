...
function mappingCard(){var a=readAnswers(),c=readCatalogue(),groups=lotGroups(c);var ext=Array.isArray(a.extensionMonths)?a.extensionMonths:[],extBudgets=Array.isArray(a.supplyExtensionBudgetsExVat)?a.supplyExtensionBudgetsExVat:[];var projected=Number(a.supplyCatalogueProjectedEstimatedValueExVat||0)||Number(a.initialBudgetExVat||0)+extBudgets.reduce(function(s,v){return s+Number(v||0);},0);var priceOnly=a.supplyAwardCriteriaMode==="PRICE_ONLY";var criteria=priceOnly?"Precio como criterio único":"Pluralidad de criterios objetivos cuantificables mediante fórmula";
var memoria=[
...
'<strong>Base económica del catálogo:</strong> '+money(c.total)+' para '+Number(a.supplyCatalogueReferenceMonths||0)+' meses. La proyección temporal de consumo ('+money(projected)+') no debe confundirse con el presupuesto máximo contractual cuando el suministro se configure en función de las necesidades.',
...
var pcap=[
'<strong>Anexo I / características:</strong> objeto, CPV, lote(s), presupuesto máximo, valor estimado, duración y prórrogas.',
