...
function section(title,status,items){return '<div class="card" style="margin-top:10px"><h4>'+esc(title)+'</h4><div class="'+(status==="READY"?'info':status==="VERIFY"?'warning':'warning')+'"><strong>'+(status==="READY"?'Preparado para trasladar':status==="VERIFY"?'Requiere verificación antes de cerrar':'Dato pendiente')+'</strong></div><ul>'+items.map(function(x){return '<li>'+x+'</li>';}).join("")+'</ul></div>';}
function mappingCard(){var a=readAnswers(),c=readCatalogue(),groups=lotGroups(c);var ext=Array.isArray(a.extensionMonths)?a.extensionMonths:[],extBudgets=Array.isArray(a.supplyExtensionBudgetsExVat)?a.supplyExtensionBudgetsExVat:[];var vec=Number(a.supplyCatalogueProjectedEstimatedValueExVat||0)||Number(a.initialBudgetExVat||0)+extBudgets.reduce(function(s,v){return s+Number(v||0);},0);var criteria=a.supplyAwardCriteriaMode==="PRICE_ONLY"?"Precio como criterio único":"Pluralidad de criterios objetivos cuantificables mediante fórmula";var criteriaStatus=a.supplyAwardCriteriaMode==="PRICE_ONLY"?"VERIFY":"READY";
var memoria=[
...
'<strong>Configuración económica:</strong> base de catálogo '+money(c.total)+' para '+Number(a.supplyCatalogueReferenceMonths||0)+' meses; presupuesto inicial '+money(a.initialBudgetExVat)+'; valor estimado '+money(vec)+'.',
'<strong>Duración:</strong> '+Number(a.initialDurationMonths||0)+' meses'+(ext.length?' + '+ext.map(function(m,i){return 'prórroga '+(i+1)+' de '+m+' meses ('+money(extBudgets[i])+')';}).join('; '):' sin prórrogas')+'.',
'<strong>Procedimiento:</strong> propuesta de abierto simplificado abreviado, pendiente de comprobación normativa final.',
'<strong>Criterios:</strong> '+criteria+'.'
];
var pcap=[
'<strong>Anexo I / características:</strong> objeto, CPV, lote(s), presupuesto, valor estimado, duración y prórrogas.',
...
'<strong>Criterios de adjudicación:</strong> '+criteria+'.',
...
Object.keys(groups).forEach(function(name){ppt.push('<strong>'+esc(name)+':</strong> '+groups[name].length+' artículos · '+money(groups[name].reduce(function(s,x){var q=Number(String(x.cantidad||"").replace(",",".")),p=Number(String(x.precio||"").replace(",","."));return s+(Number.isFinite(q)&&Number.isFinite(p)?q*p:0);},0))+'. '+rowList(groups[name]));});
...
html+=section('PCAP · Anexo I y cláusulas variables',criteriaStatus,pcap);
html+=section('PPT · prescripciones y anexo de artículos','READY',ppt);
if(criteriaStatus==="VERIFY")html+='<div class="warning"><strong>Control jurídico pendiente sobre el criterio único de precio.</strong> La configuración se conservará, pero antes de generar el PCAP debe verificarse que el uso de un único criterio resulta admisible y suficientemente motivado para este suministro concreto. Si existen otros elementos variables relevantes, el sistema deberá proponer pluralidad de criterios automáticos.</div>';
if(a.supplyDocumentMappingValidated===true)html+='<div class="info"><strong>Mapeo documental validado.</strong> El siguiente paso será seleccionar/verificar el modelo oficial vigente de PCAP de suministros y generar los borradores editables desde esta única fuente de verdad.</div>';
