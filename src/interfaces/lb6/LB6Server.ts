...
async function createCase(mode){const r=await api('/api/cases',{method:'POST',headers:headers(),body:JSON.stringify({mode})});const c=await r.json();caseId=c.id;await refresh();}
function downloadBlank(){location.href='/api/questionnaire';}
...
async function answer(id){await api('/api/cases/'+encodeURIComponent(caseId)+'/answers',{method:'POST',headers:headers(),body:JSON.stringify({questionId:id,value:document.getElementById('ans').value})});await refresh();}
