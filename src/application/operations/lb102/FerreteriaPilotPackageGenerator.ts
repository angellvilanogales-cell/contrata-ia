...
export function ferreteriaInterventionConsistencyAudit(pcapText:string,memoryText:string,pptText:string):string[]{
 ...
 if(!/informe\s+del\s+letrado/i.test(pcapText))blockers.push("INTERVENCION: PCAP no acredita consignación del informe del letrado.");
 if(/otros?\s+art[ií]culos?.{0,120}no\s+contemplad/i.test(memoryText))blockers.push("INTERVENCION: Memoria mantiene incorporación de artículos no contemplados en el listado.");
 ...
 if(!/presupuesto\s+base\s+de\s+licitaci[oó]n/i.test(pcapText))blockers.push("INTERVENCION: PCAP no explicita la referencia al presupuesto base de licitación para la baja superior al 25%.");
 if(/5\s+d[ií]as\s+h[aá]biles/i.test(pptText))blockers.push("INTERVENCION: PPT mantiene 5 días hábiles para sustituir material defectuoso.");
 if(!/3\s+d[ií]as\s+h[aá]biles/i.test(pptText)||!/3\s+d[ií]as\s+h[aá]biles/i.test(pcapText))blockers.push("INTERVENCION: PCAP y PPT no acreditan conjuntamente 3 días hábiles para material defectuoso.");
 if(/adjudicaci[oó]n\s+del\s+acuerdo\s+marco/i.test(pptText))blockers.push("INTERVENCION: PPT conserva referencia improcedente a adjudicación del acuerdo marco.");
 if(/tres\s*\(3\)\s*a[nñ]os\s+para\s+cada\s+uno\s+de\s+los\s+suministros/i.test(pptText))blockers.push("INTERVENCION: PPT mantiene garantía genérica de tres años para todos los suministros.");
...
function crossAudit(pcap:Uint8Array,memory:Uint8Array,ppt:Uint8Array):string[]{
 ...
 for(const required of ["21.793,15","18.160,96"]){if(!pcapText.includes(required))blockers.push(`PCAP: falta magnitud económica protegida ${required}.`);if(!memoryText.includes(required))blockers.push(`MEMORIA: falta magnitud económica protegida ${required}.`);}
