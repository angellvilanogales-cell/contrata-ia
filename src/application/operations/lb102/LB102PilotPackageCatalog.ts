...
export const LB102_PILOT_PACKAGE_CATALOG:readonly LB102PilotPackageDescriptor[]=[
 ...
 {id:"supply-panda",caseId:"CONTR 2025 466864",family:"SUPPLY",label:"Panda / licencias software · ASO",profile:"PANDA_SOURCE_BACKED_REGRESSION_LB102_V5"},
 {id:"service-huelva",caseId:"CONTR 2025 0000468715",family:"SERVICE",label:"Limpieza SAE Huelva",profile:"SERVICE_HUELVA_SOURCE_BACKED_REGRESSION_LB102_V1"},
 {id:"service-sevilla",caseId:"CONTR 2026 38892",family:"SERVICE",label:"Mantenimiento integral SAE Sevilla",profile:"SERVICE_SEVILLA_SOURCE_BACKED_REGRESSION_LB102_V1"},
] as const;
...
export async function generateLB102PilotPackage(id:LB102PilotPackageId):Promise<GeneratedLB102PilotPackage>{
 ...
 try{
  ...
  if(id==="supply-panda"){
   const store=createLB102PandaTemplateStoreFromEnv();if(!store)throw new Error("Persistencia Panda source-backed V5 no configurada.");
  ...
  const store=createLB102ServiceSourceBackedTemplateStoreFromEnv();if(!store)throw new Error("Persistencia Service source-backed Huelva/Sevilla no configurada.");
  const snapshot=id==="service-huelva"?LB102_SERVICE_HUELVA:LB102_SERVICE_SEVILLA;const out=await generateServiceSourceBackedPilotPackage({snapshot,templateStore:store});
