...
export type PilotAcceptanceEvent=
 | {type:"CASE_RUN";timestamp:string;actorId:string;caseId:string;family:"SUPPLY"|"SERVICE";sourceAuthority:string;packageSha256:string;humanReviewed:boolean;result:"PASS"|"FAIL";notes?:string}
...
export class PilotAcceptanceRegistry{
  ...
  public summarize(){const events=this.readAll();const caseRuns=events.filter((e):e is Extract<PilotAcceptanceEvent,{type:"CASE_RUN"}=>e.type==="CASE_RUN"&&e.result==="PASS");const sessions=events.filter((e):e is Extract<PilotAcceptanceEvent,{type:"USER_SESSION"}=>e.type==="USER_SESSION"&&e.result==="PASS");const regressions=events.filter((e):e is Extract<PilotAcceptanceEvent,{type:"NEGATIVE_REGRESSION"}=>e.type==="NEGATIVE_REGRESSION");const latestDefect=new Map<string,Extract<PilotAcceptanceEvent,{type:"DEFECT"}>>();for(const e of events)if(e.type==="DEFECT")latestDefect.set(e.defectId,e);const lastDecision=[...events].reverse().find((e):e is Extract<PilotAcceptanceEvent,{type:"ACCEPTANCE_DECISION"}=>e.type==="ACCEPTANCE_DECISION");return{ supplyRealCaseRuns:caseRuns.filter(x=>x.family==="SUPPLY").length, serviceRealCaseRuns:caseRuns.filter(x=>x.family==="SERVICE").length, generatedPackagesHumanReviewed:caseRuns.filter(x=>x.humanReviewed).length, userAcceptanceSessions:sessions.length, distinctPilotUsers:new Set(sessions.map(x=>x.actorId)).size, negativeRegressionConflictPassed:regressions.some(x=>x.check==="SOURCE_CONFLICT"&&x.result==="PASS"), negativeRegressionMissingValidationPassed:regressions.some(x=>x.check==="MISSING_HUMAN_VALIDATION"&&x.result==="PASS"), negativeRegressionTemplateIntegrityPassed:regressions.some(x=>x.check==="TEMPLATE_INTEGRITY"&&x.result==="PASS"), criticalDefectsOpen:[...latestDefect.values()].filter(x=>x.severity==="CRITICAL"&&x.state==="OPEN").length, acceptanceDecisionRecorded:lastDecision?.decision==="ACCEPT_FOR_PILOT"};}
}
