...
function uint16(value: number): Buffer { const buffer = Buffer.alloc(2); buffer.writeUInt16LE(value & 0xffff, 0); return buffer; }
function uint32(value: number): Buffer { const buffer = Buffer.alloc(4); buffer.writeUInt32LE(value >>> 0, 0); return buffer; }
...
function createStoreZip(entries: readonly ZipEntry[]): Uint8Array {
  ...
  for (const entry of entries) {
    ...
    const local = Buffer.concat([
      Buffer.from([0x50, 0x4b, 0x03, 0x04]), uint16(20), uint16(0x0800), uint16(0), uint16(dosTime), uint16(dosDate),
      uint32(checksum), uint32(data.length), uint32(data.length), uint16(name.length), uint16(0), name, data
    ]);
    localParts.push(local);
    centralParts.push(Buffer.concat([
      Buffer.from([0x50, 0x4b, 0x01, 0x02]), uint16(20), uint16(20), uint16(0x0800), uint16(0), uint16(dosTime), uint16(dosDate),
      uint32(checksum), uint32(data.length), uint32(data.length), uint16(name.length), uint16(0), uint16(0), uint16(0), uint16(0), uint32(0), uint32(offset), name
    ]));
    offset += local.length;
  }
  const centralDirectory = Buffer.concat(centralParts);
  const end = Buffer.concat([
    Buffer.from([0x50, 0x4b, 0x05, 0x06]), uint16(0), uint16(0), uint16(entries.length), uint16(entries.length),
...
/**
 * El documento administrativo visible no expone IDs de fuentes internas, nombres de
 * ficheros ni estados técnicos de validación. Esa trazabilidad permanece en el paquete
 * estructurado y en auditoría. Las referencias normativas que deban figurar en el pliego
 * forman parte del propio texto jurídico (por ejemplo, «artículo 116 LCSP»).
 */
function documentXml(document: AdministrativeDocument): string {
  const body: string[] = [paragraphXml(document.title, "Title")];
  for (const section of document.sections) {
    body.push(paragraphXml(section.heading, "Heading1"));
    for (const paragraph of section.paragraphs) body.push(paragraphXml(paragraph.text));
  }
  if (document.warnings.length > 0) {
    body.push(paragraphXml("ADVERTENCIAS PARA REVISIÓN PREVIA", "Heading1"));
...
function createDocx(document: AdministrativeDocument): Uint8Array {
  return createStoreZip([
    ...
    { name: "docProps/core.xml", data: Buffer.from(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>${xmlEscape(document.title)}</dc:title><dc:creator>Contrata-IA</dc:creator><cp:keywords>Junta de Andalucía; contratación pública</cp:keywords><dcterms:created xsi:type="dcterms:W3CDTF">2026-08-07T00:00:00Z</dcterms:created></cp:coreProperties>`, "utf8") },
    { name: "docProps/app.xml", data: Buffer.from(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>Contrata-IA</Application><AppVersion>0.1.0</AppVersion></Properties>`, "utf8") }
  ]);
}

function pdfEscape(value: string): string { return value.replaceAll("\\", "\\\\").replaceAll("(", "\\(").replaceAll(")", "\\)").replace(/[\u0100-\uffff]/g, "?"); }
...
function createPdf(document: AdministrativeDocument): Uint8Array {
  ...
  for (const section of document.sections) {
    ...
    for (const paragraph of section.paragraphs) { lines.push(...wrap(paragraph.text)); lines.push(""); }
  }
  const pages: string[][] = [];
  ...
  addObject("<< /Type /Catalog /Pages 2 0 R >>"); objects.push("");
  const fontId = addObject("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>");
  const pageIds: number[] = [];
  for (const pageLines of pages) {
    const commands = ["BT", "/F1 9 Tf", "50 800 Td", "12 TL"];
    for (const [index, line] of pageLines.entries()) { if (index > 0) commands.push("T*"); commands.push(`(${pdfEscape(line)}) Tj`); }
    commands.push("ET");
    const stream = commands.join("\n");
    const contentId = addObject(`<< /Length ${Buffer.byteLength(stream, "latin1")} >>\nstream\n${stream}\nendstream`);
    pageIds.push(addObject(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontId} 0 R >> >> /Contents ${contentId} 0 R >>`));
  }
  ...
  for (let index = 0; index < objects.length; index += 1) {
    const id = index + 1; offsets[id] = position;
    const chunk = Buffer.from(`${id} 0 obj\n${objects[index]}\nendobj\n`, "latin1"); chunks.push(chunk); position += chunk.length;
  }
  const xrefOffset = position;
  const xref = ["xref", `0 ${objects.length + 1}`, "0000000000 65535 f ", ...Array.from({ length: objects.length }, (_, index) => `${String(offsets[index + 1]).padStart(10, "0")} 00000 n `)];
...
export class AdministrativeDocumentRenderer {
  public render(packageValue: LB5DocumentPackage): LB5RenderedPackage {
    if (!packageValue.globalValidation.valid) throw new Error(`Paquete documental LB-5 inválido: ${packageValue.globalValidation.errors.join("; ")}`);
    const editable: EditableDocumentArtifact[] = [];
    const pdf: EditableDocumentArtifact[] = [];
    for (const document of packageValue.documents) {
      editable.push({ documentId: document.id, fileName: `${document.fileBaseName}.docx`, mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", data: createDocx(document) });
      pdf.push({ documentId: document.id, fileName: `${document.fileBaseName}.pdf`, mimeType: "application/pdf", data: createPdf(document) });
    }
