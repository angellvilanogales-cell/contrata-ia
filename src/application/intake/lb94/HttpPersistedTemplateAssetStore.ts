...
function normalizeEndpoint(value: string): string { const endpoint=value.trim().replace(/\/+$/,""); if(!endpoint.startsWith("https://"))throw new Error("El almacén remoto de plantillas LB94 exige HTTPS."); return endpoint; }
function decodeBase64(value: string): Uint8Array { if(!/^[A-Za-z0-9+/]*={0,2}$/.test(value)||value.length%4!==0)throw new Error("El activo remoto contiene base64 inválido."); return Buffer.from(value,"base64"); }
...
/** Store físico universal de plantillas persistidas. VIABILITY se incorpora en LB98 sin alterar los activos históricos. */
export class HttpPersistedTemplateAssetStore implements UniversalEditableTemplateBinaryStore {
  private readonly endpoint:string;
  public constructor(endpoint:string,private readonly token:string,private readonly manifest:readonly PersistedTemplateAssetDescriptor[]=LB94_SUPPLY_GENERAL_RUNTIME_ASSETS){this.endpoint=normalizeEndpoint(endpoint);if(!token.trim())throw new Error("Falta token del almacén remoto de plantillas LB94.");}
  public async get(templateId:string):Promise<UniversalEditableTemplateBinarySource|null>{
    const descriptor=this.manifest.find(item=>item.templateId===templateId);if(!descriptor)return null;
    const response=await fetch(`${this.endpoint}/templates/${encodeURIComponent(templateId)}`,{method:"GET",headers:{"x-contrata-ia-persistence-token":this.token,accept:"application/json"}});if(response.status===404)return null;if(!response.ok)throw new Error(`No se puede recuperar ${descriptor.kind} desde persistencia: HTTP ${response.status}.`);
    const payload=await response.json() as RemoteTemplatePayload;if(payload.templateId!==descriptor.templateId)throw new Error(`Identidad remota incorrecta para ${descriptor.kind}.`);if(payload.kind!==descriptor.kind)throw new Error(`Naturaleza remota incorrecta para ${descriptor.templateId}.`);if(payload.mediaType!=="application/vnd.oasis.opendocument.text")throw new Error(`Media type remoto inválido para ${descriptor.templateId}.`);if(payload.sha256!==descriptor.sha256)throw new Error(`SHA declarado remoto no coincide para ${descriptor.templateId}.`);if(payload.styleFingerprint!==descriptor.styleFingerprint)throw new Error(`Huella de estilo remota no coincide para ${descriptor.templateId}.`);if(provenanceRole(payload.provenance)!==descriptor.provenanceRole)throw new Error(`Procedencia remota no coincide para ${descriptor.templateId}.`);if(typeof payload.contentBase64!=="string")throw new Error(`Faltan bytes persistidos para ${descriptor.templateId}.`);const bytes=decodeBase64(payload.contentBase64);if(typeof payload.byteLength!=="number"||payload.byteLength!==bytes.byteLength)throw new Error(`Longitud remota no coincide para ${descriptor.templateId}.`);if(sha256(bytes)!==descriptor.sha256)throw new Error(`SHA calculado de los bytes remotos no coincide para ${descriptor.templateId}.`);return{templateId:descriptor.templateId,sourceId:descriptor.sourceId,bytes};
  }
