...
export async function loadPersistedSupplyGeneralTemplate(
  ...
  return {
    ...
    derivedSha256: createHash("sha256").update(source.bytes).digest("hex"),
    derivedStyleFingerprint: style,
    derivedStructuralStyleFingerprint: structuralStyle,
    corpusSourceIds: SUPPLY_GENERAL_STRUCTURAL_CORPUS,
    transformationVersion: descriptor.derivationVersion,
    contaminationHits: [],
    bytes: source.bytes,
