...
export interface WorksTemplatePhysicalGateResult {
  ready: boolean;
  actualSha256: string;
...
export function evaluateWorksGeneralTemplateBytes(manifest: WorksGeneralTemplateManifestRecord, bytes: Uint8Array): WorksTemplatePhysicalGateResult {
  ...
  if(actualSha256!==manifest.expectedSha256) blockers.push(`SHA-256 incorrecto para ${manifest.kind} Works.`);
  try {
    const entries=readOdtZip(bytes);
    const mime=Buffer.from(entry(entries,"mimetype").bytes).toString("utf8").trim();
    if(mime!==manifest.mediaType) blockers.push(`Media type incorrecto para ${manifest.kind} Works.`);
    const content=Buffer.from(entry(entries,"content.xml").bytes).toString("utf8");
    entry(entries,"styles.xml"); entry(entries,"META-INF/manifest.xml");
    discoveredSlots=[...content.matchAll(/\{\{([A-Za-z0-9.]+)\}\}/g)].map(m=>m[1]!).sort();
    if(!same(discoveredSlots,manifest.slots)) blockers.push(`Inventario de slots incorrecto para ${manifest.kind} Works.`);
    ...
    if(actualStyleFingerprint!==manifest.expectedStyleFingerprint) blockers.push(`Huella de estilo incorrecta para ${manifest.kind} Works.`);
  } catch(error){blockers.push(error instanceof Error?error.message:String(error));}
  if(manifest.officialModel!==false||manifest.provenance!=="CONTRATA_IA_DERIVED_GENERAL_TEMPLATE") blockers.push("Procedencia Works incompatible con la política LB97.");
