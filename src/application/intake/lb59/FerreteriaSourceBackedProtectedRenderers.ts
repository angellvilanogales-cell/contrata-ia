...
export const FERRETERIA_PPT_PHYSICAL_BINDING_INVENTORY: readonly ProtectedPhysicalBindingInventoryItem[] = [
  ...
  { id: "ppt.catalogue-98-source-backed", part: "content.xml", sourceAnchor: "4. DESCRIPCIÓN DE LOS MATERIALES OBJETO DEL PRESENTE CONTRATO", effect: "Audita que las 98 referencias canónicas source-backed permanecen presentes sin divergencias." },
] as const;
...
function part(entries: readonly OdtZipEntry[], name: string): string {
...
function count(text: string, token: string): number {
  ...
  let total = 0;
  let cursor = 0;
  while ((cursor = text.indexOf(token, cursor)) >= 0) { total += 1; cursor += token.length; }
  return total;
}

function replaceUnique(text: string, oldValue: string, newValue: string, label: string): string {
  const occurrences = count(text, oldValue);
  if (occurrences !== 1) throw new Error(`${label}: el anclaje físico aparece ${occurrences} veces; se exige coincidencia única.`);
  return text.replace(oldValue, newValue);
}

function paragraphContaining(text: string, needle: string): { value: string; start: number; end: number } {
  const pattern = /<text:p\b[^>]*>[\s\S]*?<\/text:p>/g;
  const matches = [...text.matchAll(pattern)].filter(match => match[0].includes(needle));
  if (matches.length !== 1 || matches[0]?.index === undefined) throw new Error(`Párrafo anclado por «${needle}» aparece ${matches.length} veces.`);
  const value = matches[0][0];
  const start = matches[0].index;
  return { value, start, end: start + value.length };
}

function replaceParagraph(text: string, needle: string, replacement: string): string {
  const current = paragraphContaining(text, needle);
  return text.slice(0, current.start) + replacement + text.slice(current.end);
}

function replaceParagraphSequence(text: string, firstNeedle: string, lastNeedle: string, replacement: string): string {
  const first = paragraphContaining(text, firstNeedle);
  const last = paragraphContaining(text, lastNeedle);
  if (last.start < first.start) throw new Error(`Secuencia física inválida: ${lastNeedle} precede a ${firstNeedle}.`);
  return text.slice(0, first.start) + replacement + text.slice(last.end);
}

function replaceNamedTable(text: string, tableName: string, replacement: string): string {
  const token = `table:name=\"${tableName}\"`;
  const anchor = text.indexOf(token);
  if (anchor < 0) throw new Error(`No se localiza la tabla ${tableName}.`);
  if (text.indexOf(token, anchor + token.length) >= 0) throw new Error(`La tabla ${tableName} no es única.`);
  const start = text.lastIndexOf("<table:table ", anchor);
  const end = text.indexOf("</table:table>", anchor);
  if (start < 0 || end < 0) throw new Error(`No se puede delimitar físicamente la tabla ${tableName}.`);
  return text.slice(0, start) + replacement + text.slice(end + "</table:table>".length);
}
...
function structuralStyleFingerprint(entries: readonly OdtZipEntry[]): string {
  const styles = part(entries, "styles.xml").replace(/<office:master-styles\b[\s\S]*?<\/office:master-styles>/, "<office:master-styles/>");
  const content = part(entries, "content.xml");
  const automatic = content.match(/<office:automatic-styles\b[\s\S]*?<\/office:automatic-styles>/)?.[0] ?? "";
  ...
  return `sha256:${createHash("sha256").update([styles, automatic, settingsText].join("\n--CONTRATA-IA-STYLE-PART--\n")).digest("hex")}`;
}
...
function assertSource(source: { bytes: Uint8Array; templateId: string }, expectedTemplate: string, expectedHash: string, expectedStyle: string): OdtZipEntry[] {
  if (source.templateId !== expectedTemplate) throw new Error(`Activo incorrecto: se esperaba ${expectedTemplate}.`);
  const actualHash = hash(source.bytes);
  if (actualHash !== expectedHash) throw new Error(`SHA-256 fuente no coincide para ${expectedTemplate}.`);
  const entries = readOdtZip(source.bytes);
  if (computeOdtStyleFingerprint(entries) !== expectedStyle) throw new Error(`Huella de estilos fuente no coincide para ${expectedTemplate}.`);
  return entries;
}
...
function auditMemory(entries: readonly OdtZipEntry[], sourceStructuralStyle: string) {
  const text = visibleText(part(entries, "content.xml"));
  const styles = part(entries, "styles.xml");
  const blockers: string[] = [];
  for (const required of ["21.793,15", "18.160,96", "3.632,19", "artículo 159.4.a", "sin incorporar artículos nuevos ni establecer precios unitarios nuevos", "no se exige habilitación empresarial o profesional específica"]) {
    if (!text.includes(required)) blockers.push(`Memoria: falta contenido obligatorio «${required}».`);
  }
  for (const forbidden of ["25.325,86", "Incorporación al contrato de otros artículos no contemplados", "se exime a los licitadores de la obligación de inscribirse"]) {
    if (text.includes(forbidden)) blockers.push(`Memoria: persiste contenido incompatible «${forbidden}».`);
  }
...
export async function renderFerreteriaProtectedMemory(store: UniversalEditableTemplateBinaryStore): Promise<ProtectedCaseDocumentRender> {
  const source = await store.get(FERRETERIA_MEMORY_TEMPLATE_ID);
  if (!source) throw new Error("Memoria: no están disponibles los bytes exactos V12 en runtime.");
  let entries = assertSource(source, FERRETERIA_MEMORY_TEMPLATE_ID, MEMORY_SOURCE_SHA, MEMORY_SOURCE_STYLE);
  const sourceStructuralStyle = structuralStyleFingerprint(entries);
  let content = part(entries, "content.xml");
  ...
  const footerToken = '<text:span text:style-name="MT2">7</text:span>';
  ...
  styles = styles.split(footerToken).join('<text:span text:style-name="MT2">9</text:span>');
  entries = replacePart(entries, "styles.xml", styles);
  const audit = auditMemory(entries, sourceStructuralStyle);
  const bytes = writeOdtZip(entries);
  return {
    kind: "MEMORIA",
    fileName: "CONTR-2026-240267_Memoria_Justificativa_V14_Protegida_Contrata-IA.odt",
    bytes,
    sourceSha256: MEMORY_SOURCE_SHA,
    renderedSha256: hash(bytes),
    sourceStyleFingerprint: MEMORY_SOURCE_STYLE,
    renderedStyleFingerprint: computeOdtStyleFingerprint(entries),
    auditReady: audit.ready,
    auditBlockers: audit.blockers,
    appliedPhysicalBindings: FERRETERIA_MEMORY_PHYSICAL_BINDING_INVENTORY.map(item => item.id),
  };
}
...
export async function renderFerreteriaProtectedPpt(store: UniversalEditableTemplateBinaryStore): Promise<ProtectedCaseDocumentRender> {
  const source = await store.get(FERRETERIA_PPT_TEMPLATE_ID);
  if (!source) throw new Error("PPT: no están disponibles los bytes exactos V6 en runtime.");
  let entries = assertSource(source, FERRETERIA_PPT_TEMPLATE_ID, PPT_SOURCE_SHA, PPT_SOURCE_STYLE);
  let content = part(entries, "content.xml");
  ...
  entries = replacePart(entries, "content.xml", content);
  const audit = auditPpt(entries);
  const renderedStyle = computeOdtStyleFingerprint(entries);
  if (renderedStyle !== PPT_SOURCE_STYLE) {
    return {
      kind: "PPT",
      fileName: "CONTR-2026-240267_PPT_V7_Protegido_Contrata-IA.odt",
      bytes: writeOdtZip(entries),
      sourceSha256: PPT_SOURCE_SHA,
      renderedSha256: hash(writeOdtZip(entries)),
      sourceStyleFingerprint: PPT_SOURCE_STYLE,
      renderedStyleFingerprint: renderedStyle,
      auditReady: false,
      auditBlockers: [...audit.blockers, "PPT: el render alteró la huella de estilos del V6."],
      appliedPhysicalBindings: FERRETERIA_PPT_PHYSICAL_BINDING_INVENTORY.map(item => item.id),
    };
  }
  const bytes = writeOdtZip(entries);
  return {
...
export function evaluateFerreteriaProtectedRendererPhysicalClosure() {
  const memoryIds = new Set(FERRETERIA_MEMORY_PHYSICAL_BINDING_INVENTORY.map(item => item.id));
  const pptIds = new Set(FERRETERIA_PPT_PHYSICAL_BINDING_INVENTORY.map(item => item.id));
  const blockers: string[] = [];
