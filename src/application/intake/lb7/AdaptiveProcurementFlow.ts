...
export interface LegalGround { readonly article: string; readonly rule: string; readonly source: "LCSP" | "CPV"; readonly verification: "CURRENT_LAW_REQUIRED"; }
export interface CpvProposal { readonly code: string; readonly label: string; readonly role: "PRIMARY" | "COMPLEMENTARY"; readonly confidence: "HIGH" | "MEDIUM"; }
...
function inferNature(answers: AdaptiveFlowAnswers): { nature: ContractNature; reason: string } {
  ...
  const supplyScore = ["ferreter", "material", "suministr", "pieza", "producto", "equipamiento", "mobiliario"].filter(token => value.includes(token)).length;
  const serviceScore = ["página web", "pagina web", "sitio web", "desarroll", "diseñ", "mantenimiento", "soporte", "servicio", "consultor", "asistencia"].filter(token => value.includes(token)).length;
...
function serviceEconomics(answers: AdaptiveFlowAnswers): EconomicProjection {
  ...
  if (oneOff === undefined || annual === undefined) return { initialBudgetExVat: initialBudget, maximumDurationMonths, status: "PROVISIONAL", note: "No se proyectan todavía las prórrogas: falta completar la separación entre el coste inicial no recurrente y el coste periódico." };
  ...
  if (wholeYears > 0) {
    ...
    for (let year = 2; year <= wholeYears; year += 1) annualProjection.push({ period: `Año ${year}`, amountExVat: annual });
    const remainder = initialMonths - wholeYears * 12;
    if (remainder > 0) annualProjection.push({ period: `Periodo inicial adicional (${remainder} meses)`, amountExVat: annual * (remainder / 12) });
  } else annualProjection.push({ period: `Periodo inicial (${initialMonths} meses)`, amountExVat: oneOff + annual * (initialMonths / 12) });
  ...
  return {
    ...
    note: coherent ? "La hipótesis económica cuadra con el presupuesto inicial y permite proyectar las prórrogas sin repetir el coste inicial no recurrente." : `La distribución propuesta suma ${calculatedInitial.toFixed(2)} € para el periodo inicial y no coincide con los ${initialBudget.toFixed(2)} € declarados. Debe revisarse antes de fijar el valor estimado.`
  };
}
...
function procedureDecision(nature: ContractNature, economic: EconomicProjection, answers: AdaptiveFlowAnswers): { procedure: ProcedureProposal; reason: string; constraint: string } {
  ...
  if ((nature === "SUPPLIES" || nature === "SERVICES") && value < 60000) {
    ...
    return { procedure: "OPEN_SIMPLIFIED_ABBREVIATED_CANDIDATE", reason: "Por el valor estimado puede proponerse el procedimiento abierto simplificado abreviado, condicionado a que todos los criterios sean cuantificables mediante fórmulas y a la comprobación de los restantes requisitos legales.", constraint: "Si resulta imprescindible un criterio sometido a juicio de valor, esta propuesta debe descartarse y reconsiderarse el procedimiento." };
  }
  if ((nature === "SUPPLIES" || nature === "SERVICES") && value < 143000) return { procedure: "OPEN_SIMPLIFIED_CANDIDATE", reason: "Por cuantía puede estudiarse el procedimiento abierto simplificado, sujeto a las condiciones del artículo 159 LCSP y a la configuración de los criterios de adjudicación.", constraint: "La ponderación de criterios automáticos y de juicio de valor debe validarse antes de cerrar el procedimiento." };
  return { procedure: "OPEN_PROCEDURE_REVIEW_REQUIRED", reason: "La cuantía exige determinar el procedimiento atendiendo al valor estimado, naturaleza del contrato, eventual sujeción a regulación armonizada y demás circunstancias del expediente.", constraint: "Se requiere revisión normativa actualizada antes de fijar definitivamente el procedimiento." };
}
...
export class AdaptiveProcurementFlow {
  public analyze(answers: AdaptiveFlowAnswers): AdaptiveFlowDecision {
    ...
    return { contractNature: nature.nature, contractNatureReason: nature.reason, lotProposal: lots.proposal, lotReason: lots.reason, cpv, economics: economic, procedure: procedure.procedure, procedureReason: procedure.reason, awardCriteriaConstraint: procedure.constraint, proposals, warnings, legalGrounds, nextQuestion: nextQuestion(answers, nature.nature, lots.proposal, economic) };
  }
}