...
async function load(kind:Kind,key:CaseKey,store:UniversalEditableTemplateBinaryStore){
 const spec=CASES[key],asset=descriptor(spec.assetIds[kind]),source=await store.get(asset.templateId);if(!source)throw new Error(`Falta activo Service ${key}/${kind} V8.`);
 if(sha(source.bytes)!==asset.sha256)throw new Error(`SHA Service ${key}/${kind} V8 incorrecto.`);const entries=readOdtZip(source.bytes);if(computeOdtStyleFingerprint(entries)!==asset.styleFingerprint)throw new Error(`Huella Service ${key}/${kind} V8 incorrecta.`);
...
/** Empaqueta exclusivamente reconstrucciones físicas V8 de los casos de regresión Huelva/Sevilla; nunca actúa como modelo Service general. */
