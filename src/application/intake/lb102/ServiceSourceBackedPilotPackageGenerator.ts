...
type ServiceCaseSpec={caseId:string;objectValue:string;sourceAuthority:string;pages:Readonly<Record<Kind,number>>;sourceIdentifierVariants:readonly string[];markers:Readonly<Record<Kind,readonly string[]>>;assetIds:Readonly<Record<Kind,string>>;quality:Readonly<Record<Kind,{exact:readonly string[];semantic:readonly (readonly string[])[]}>>};
const CASES:Readonly<Record<CaseKey,ServiceCaseSpec>>={
 HUELVA:{
  caseId:"CONTR 2025 468715",objectValue:"SERVICIOS DE LIMPIEZA",sourceAuthority:"JDA_SAE_HUELVA_PRIMARY_TRIAD",pages:{MEMORIA:13,PCAP:103,PPT:28},sourceIdentifierVariants:["CONTR 2025 468715"],
 ...
 SEVILLA:{
  caseId:"CONTR 2026 38892",objectValue:"SERVICIO DE MANTENIMIENTO INTEGRAL",sourceAuthority:"JDA_SAE_SEVILLA_PRIMARY_TRIAD",pages:{MEMORIA:13,PCAP:113,PPT:53},sourceIdentifierVariants:["CONTR 2026 38892","CONTR/20026/38892"],
...
function assertSourcePcapEvidence(bytes:Uint8Array,key:CaseKey){const spec=CASES[key],q=spec.quality.PCAP,text=odtText(bytes);assertLb102NoCriticalPlaceholders(text,`Service ${key}/PCAP evidencia`);assertLb102ExactMaterialization(text,`Service ${key}/PCAP evidencia`,q.exact);assertLb102SemanticConcepts(text,`Service ${key}/PCAP evidencia`,q.semantic);if(!text.toLocaleLowerCase("es").includes(spec.objectValue.toLocaleLowerCase("es")))throw new Error(`Service ${key}/PCAP evidencia: no acredita físicamente el objeto ${spec.objectValue}.`);}
