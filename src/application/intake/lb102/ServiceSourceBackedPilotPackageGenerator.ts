...
async function load(kind:Kind,key:CaseKey,store:UniversalEditableTemplateBinaryStore){const spec=CASES[key],asset=descriptor(spec.assetIds[kind]),source=await store.get(asset.templateId);if(!source)throw new Error(`Falta activo Service ${key}/${kind} V8.`);if(sha(source.bytes)!==asset.sha256)throw new Error(`SHA Service ${key}/${kind} V8 incorrecto.`);const entries=readOdtZip(source.bytes);if(computeOdtStyleFingerprint(entries)!==asset.styleFingerprint)throw new Error(`Huella Service ${key}/${kind} V8 incorrecta.`);const sourceText=odtText(source.bytes);if(/\{\{[^}]+\}\}|DATOS VARIABLES DEL EXPEDIENTE/.test(sourceText))throw new Error(`Service ${key}/${kind} conserva marcadores técnicos.`);for(const marker of spec.markers[kind])if(!sourceText.toLowerCase().includes(marker.toLowerCase()))throw new Error(`Service ${key}/${kind}: falta marcador físico ${marker}.`);
 // Lección Panda: una fuente real firmada es evidencia, nunca salida del generador.
 const sanitized=sanitizeOdtSignatureResidue(source.bytes);const q=spec.quality[kind];assertLb102GeneratedDocumentQuality({bytes:sanitized,kind,label:`Service ${key}/${kind}`,exactValues:q.exact,semanticConceptGroups:q.semantic});return sanitized;}

/**
 * Lección Panda aplicada antes de UAT humana: un PCAP derivado de un expediente
 * real no puede aceptarse como plantilla final si existe un modelo oficial Junta.
 * Huelva y Sevilla quedan bloqueados antes de que el usuario tenga que descubrirlo.
 */
export function assertServiceOfficialPcapPreUat(key:CaseKey){throw new Error(`Service ${key}: UAT bloqueada preventivamente. El PCAP V8 procede de un expediente real (officialModel=false). Debe reconstruirse sobre el modelo oficial Junta de Servicios correspondiente antes de revisión humana.`);}
...
export async function generateServiceSourceBackedPilotPackage(input:{snapshot:StrictServicePilotSnapshot;templateStore:UniversalEditableTemplateBinaryStore}){
 try{
  ...
  const [pcap,memoria,ppt]=await Promise.all([load("PCAP",key,input.templateStore),load("MEMORIA",key,input.templateStore),load("PPT",key,input.templateStore)]);
  // No permitimos que Huelva/Sevilla lleguen a UAT con el defecto estructural ya detectado en Panda.
  assertServiceOfficialPcapPreUat(key);
