...
const HARD_MARKERS=[/FIRMADO POR/i,/C[ÓO]DIGO SEGURO DE VERIFICACI[ÓO]N/i,/ws050\.juntadeandalucia\.es\/verificarFirma/i] as const;
...
function isSignedIdentityLine(line:string){const text=plain(line);return /FIRMADO POR/i.test(text)||(/\b\d{2}\/\d{2}\/\d{4}(?:\s+\d{2}:\d{2}:\d{2})?\b/.test(text)&&/P[ÁA]G(?:INA)?\.?\s*[:.]?\s*\d+/i.test(text));}
...
function sanitizeParagraphBody(body:string){
 ...
 for(let i=0;i<parts.length;i+=1){
  const line=parts[i]??"";
  if(isAuthenticCopyLine(line)||isVerificationIntro(line)||isVerificationLine(line)||/FIRMADO POR/i.test(plain(line)))drop.add(i);
...
export function findOdtSignatureResidue(bytes:Uint8Array):string[]{
 ...
 if(content){const xml=Buffer.from(content.bytes).toString("utf8");for(const marker of HARD_MARKERS)if(marker.test(xml))findings.push(`marca visible ${marker.source}`);if(/VERIFICACI[ÓO]N[\s\S]{0,180}P[ÁA]G(?:INA)?/i.test(plain(xml)))findings.push("bloque visible de verificación/paginación");}
