...
async function render(kind:Kind,record:UniversalEvidenceRecord,store:UniversalEditableTemplateBinaryStore){
 const spec=SPECS[kind],source=await store.get(spec.asset.templateId);if(!source)throw new Error(`Falta activo Panda ${kind} V3.`);
 if(sha(source.bytes)!==spec.asset.sha256)throw new Error(`SHA Panda ${kind} V3 incorrecto.`);
 let entries=readOdtZip(source.bytes);if(computeOdtStyleFingerprint(entries)!==spec.asset.styleFingerprint)throw new Error(`Huella Panda ${kind} V3 incorrecta.`);
...
export async function generatePandaSourceBackedPilotPackage(input:{record:UniversalEvidenceRecord;templateStore:UniversalEditableTemplateBinaryStore}){
 try{
  if(input.record.caseId!=="CONTR 2025 466864")throw new Error("El renderer Panda source-backed V3 solo admite el caso de regresión CONTR 2025 466864.");
