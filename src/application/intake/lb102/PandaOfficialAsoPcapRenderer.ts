...
export async function loadPandaOfficialAsoPcapForCalibration(store:UniversalEditableTemplateBinaryStore){
 ...
 const body=text(source.bytes);const required=["Suministro","Abierto Simplificado ordinario","ELEMENTOS DEL CONTRATO"];
...
/**
 * Nunca degrada al PCAP firmado del expediente ni al derivado V1. Mientras no
 * se hayan calibrado los anclajes del Anexo I contra el binario oficial exacto,
 * el resultado permanece bloqueado de forma explícita.
 */
export async function renderPandaOfficialAsoPcap(input:{templateStore:UniversalEditableTemplateBinaryStore;caseId:string}):Promise<PandaOfficialAsoPcapRenderResult>{
 const source=await loadPandaOfficialAsoPcapForCalibration(input.templateStore);void source;void input.caseId;
