...
function requireTerm(label:string,term:string,targets:readonly {name:string;text:string}[]){if(!term.trim())throw new Error(`Auditoría cruzada ASO software: ${label} vacío.`);for(const target of targets)if(!target.text.includes(term))throw new Error(`Auditoría cruzada ASO software: ${target.name} no materializa ${label}=${term}.`);}
...
export async function generateSupplyAsoUserDocumentPackage(input:{record:UniversalEvidenceRecord;templateStore:UniversalEditableTemplateBinaryStore}){
 try{
  ...
  const docs={PCAP:odtText(pcap.bytes),MEMORIA:odtText(memory.bytes),PPT:odtText(ppt.bytes)};
  const all=[{name:"PCAP",text:docs.PCAP},{name:"MEMORIA",text:docs.MEMORIA},{name:"PPT",text:docs.PPT}] as const;
  const admin=[{name:"PCAP",text:docs.PCAP},{name:"MEMORIA",text:docs.MEMORIA}] as const;
  requireTerm("caseId",input.record.caseId,all);
  requireTerm("objeto",object,all);
  requireTerm("CPV",cpv,all);
  requireTerm("PBL",eur(c.pbl),admin);
  requireTerm("VE",eur(c.ve),admin);
  if(all.some(x=>/ferreter[ií]a/i.test(x.text)))throw new Error("Contaminación de subfamilia: el paquete software contiene referencias de ferretería.");
  const safe=input.record.caseId.replaceAll("/","-");const docsOut=[{kind:"PCAP" as const,fileName:pcap.fileName,bytes:pcap.bytes,sha256:pcap.sha256,provenance:"CONTRATA_IA_DERIVED_PROCEDURE_TEMPLATE",officialModel:false as const},{kind:"MEMORIA" as const,fileName:memory.fileName,bytes:memory.bytes,sha256:memory.sha256,provenance:"CONTRATA_IA_DERIVED_SUBFAMILY_TEMPLATE",officialModel:false as const},{kind:"PPT" as const,fileName:ppt.fileName,bytes:ppt.bytes,sha256:ppt.sha256,provenance:"CONTRATA_IA_DERIVED_SUBFAMILY_TEMPLATE",officialModel:false as const}];
  const manifest={schemaVersion:1,caseId:input.record.caseId,profile:"SUPPLY_ASO_SOFTWARE_AUTOFINANCED_LB102" as const,sourceAuthority:"JDA_RECOMMENDED_SUPPLY_ASO_AUTOFINANCED_2025_12_PLUS_REG_SUPPLY_002",documents:docsOut.map(({kind,fileName,sha256,provenance,officialModel})=>({kind,fileName,sha256,provenance,officialModel})),crossDocumentAuditReady:true,sourceRegression:"REG-SUPPLY-002",humanAcceptanceRequired:true as const,productionReady:false as const};
  const bytes=zipStoredFiles([...docsOut.map(d=>({name:d.fileName,bytes:d.bytes})),{name:"manifest.json",bytes:Buffer.from(JSON.stringify(manifest,null,2),"utf8")}]);return{ready:true,fileName:`Contrata-IA_${safe}_Supply_ASO_Software.zip`,bytes,sha256:sha(bytes),manifest,blockers:[] as string[]};
