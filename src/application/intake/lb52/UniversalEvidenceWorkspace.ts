...
function source(kind: EvidenceReference["kind"], sourceId: string): EvidenceReference {
  return { kind, sourceId };
}

function assertAllowed(fieldPath: string): void {
  if (!ALLOWED_PATHS.has(fieldPath)) throw new Error(`Campo universal no editable desde V1: ${fieldPath}.`);
}
...
export class UniversalEvidenceWorkspace {
  ...
  public declare(caseId: string, fieldPath: string, value: unknown, actor: string): UniversalEvidenceRecord {
    ...
    const field: EvidenceField<unknown> = {
      key: fieldPath,
      value,
      status: "SOURCE_DECLARED",
      sources: [source("USER_INPUT", `ui:${actor}`)],
      humanValidationRequired: true,
      humanValidated: false,
      diagnostics: ["Valor declarado desde la interfaz universal; requiere revisión humana antes de producción."],
    };
  ...
  public setSourceEvidence(caseId: string, fieldPath: string, value: unknown, status: Extract<EvidenceFieldStatus, "SOURCE_CONFIRMED" | "SOURCE_CONFLICT" | "NOT_APPLICABLE">, references: readonly EvidenceReference[], diagnostics: readonly string[] = []): UniversalEvidenceRecord {
    ...
    const field: EvidenceField<unknown> = {
      ...
      status,
      sources: references,
      humanValidationRequired: status !== "NOT_APPLICABLE",
      humanValidated: false,
      diagnostics,
      ...(status === "SOURCE_CONFLICT" ? { conflict: { statements: diagnostics.length ? diagnostics : ["Fuentes incompatibles."], treatment: "DO_NOT_AUTO_RESOLVE" as const } } : {}),
    };
    return this.write(caseId, { ...current.fields, [fieldPath]: field });
  }
  ...
  public validate(caseId: string, fieldPath: string, reviewer: string): UniversalEvidenceRecord {
    assertAllowed(fieldPath);
    const current = this.get(caseId);
    const existing = current.fields[fieldPath];
    ...
    const validated: EvidenceField<unknown> = {
      ...existing,
      status: existing.status === "NOT_APPLICABLE" ? "NOT_APPLICABLE" : "HUMAN_VALIDATED",
      humanValidated: true,
      sources: [...existing.sources, source("USER_INPUT", `review:${reviewer}`)],
      diagnostics: [...(existing.diagnostics ?? []), `Validación humana expresa realizada por ${reviewer}.`],
    };
  ...
  private write(caseId: string, fields: Readonly<Record<string, EvidenceField<unknown>>>): UniversalEvidenceRecord {
    const value: UniversalEvidenceRecord = { caseId, fields, updatedAt: new Date().toISOString() };
    const file = this.fileFor(caseId);
    const tmp = `${file}.tmp`;
